...
function SendRequests() { #model_name
    ...
    logname=serving_${model_name#*L}_$port.log
    ...
    for ((i=$start*10; i<=$end*10; i+=$interval)); do
        ...
        python benchmarks/benchmark_serving.py --backend vllm --tokenizer ../Llama-2-7b-hf \
...
function LaunchServer() { #model_name
    ...
    CUDA_VISIBLE_DEVICES=${cuda_device} python -m vllm.entrypoints.api_server --model ${model_name} --swap-space 16 --disable-log-requests --backend=$backend --port=$port -tp=${TP} ${extra_flag}
}
...
function BenchmarkModel() { #model_name
	...
	port=${ports[idx]}
        ...
        LaunchServer $model_name $backend $port ${cuda_device}>> server_$backend.log 2>&1 & pids+=( "$!" )
    done
